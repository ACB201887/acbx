...
// Copyright (c) 2009-2016 The BBC coin bt developers
...
UniValue generate(const JSONRPCRequest& request)
{
	...
	//std::string kStrMD5 = md5(str.substr(0,6)+"BBC coin bt");
	
	//if(str.substr(6,12)!=kStrMD5.substr(0,6)){
...
UniValue estimatesmartfee(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"feerate\" : x.x,     (numeric) estimate fee-per-kilobyte (in BBC)\n"
